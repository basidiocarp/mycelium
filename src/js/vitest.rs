//! Vitest test runner filter that shows only failures with ANSI stripping.
...
/// Supported Vitest subcommands.
#[derive(Debug, Clone)]
pub enum VitestCommand {
    Run,
}

/// Execute a Vitest command with token-optimized output filtering.
...
fn run_vitest(args: &[String], verbose: u8) -> Result<()> {
    ...
    let output = crate::dispatch::exec::run_bounded(&mut cmd).context("Failed to run vitest")?;
