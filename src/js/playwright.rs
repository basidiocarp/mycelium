//! Playwright E2E test filter that shows only failures grouped by suite.
use crate::tracking;
...
/// Parser for Playwright JSON reporter output into test results.
...
fn extract_playwright_regex(output: &str) -> Option<TestResult> {
    ...
    for caps in pw_summary_re().captures_iter(&clean_output) {
    ...
    // Parse duration
    let duration_ms = pw_duration_re().captures(&clean_output).and_then(|caps| {
...
fn extract_failures_regex(output: &str) -> Vec<TestFailure> {
    ...
    for caps in pw_test_pattern().captures_iter(output) {
...
/// Run Playwright and filter output to show only failures grouped by suite.
