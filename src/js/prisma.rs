//! Prisma CLI filter that strips ASCII art and verbose output.
use crate::tracking;
use anyhow::{Context, Result};
use std::process::Command;

/// Supported Prisma CLI subcommands.
#[derive(Debug, Clone)]
...
/// Prisma migrate subcommand variants (dev, status, deploy).
#[derive(Debug, Clone)]
...
/// Execute a Prisma CLI command with token-optimized output filtering.
...
fn create_prisma_command() -> Command {
    let prisma_exists = crate::utils::which_command("prisma").is_some();
