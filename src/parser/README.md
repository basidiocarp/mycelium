        ...
        ParseResult::Degraded(data, warnings) => {
            if verbose > 0 {
                for warn in warnings {
                    eprintln!("[MYCELIUM:DEGRADED] {}", warn);
                }
            }
            data.format(mode)
        }
        ParseResult::Passthrough(raw) => {
            eprintln!("[MYCELIUM:PASSTHROUGH] Parser failed, showing truncated output");
            raw
        }
    };
...
```
[MYCELIUM:DEGRADED] vitest parser: JSON parse failed at line 42, using regex fallback
[MYCELIUM:PASSTHROUGH] playwright parser: Pattern mismatch, showing truncated output
```
...
- [x] Extend tracking.db: `parse_tier`, `format_mode`
- [ ] `mycelium parse-health` command
