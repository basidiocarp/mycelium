...
//!
//! The three-tier system ensures Mycelium never returns false data silently.
...
pub trait OutputParser: Sized {
    ...
    /// 3. Return truncated passthrough with `[MYCELIUM:PASSTHROUGH]` marker
    fn parse(input: &str) -> ParseResult<Self::Output>;
}

/// Truncate output to max characters with a `[MYCELIUM:PASSTHROUGH]` marker.
pub fn truncate_output(output: &str, max_chars: usize) -> String {
    ...
    format!(
        "{}\n\n[MYCELIUM:PASSTHROUGH] Output truncated ({} chars → {} chars)",
        truncated,
        chars.len(),
        max_chars
    )
...
/// Emit a Tier 2 degradation warning to stderr for partial parse results.
pub fn emit_degradation_warning(tool: &str, reason: &str) {
    eprintln!("[MYCELIUM:DEGRADED] {} parser: {}", tool, reason);
}

/// Emit a Tier 3 passthrough warning to stderr when parsing fails entirely.
pub fn emit_passthrough_warning(tool: &str, reason: &str) {
    eprintln!("[MYCELIUM:PASSTHROUGH] {} parser: {}", tool, reason);
}
...
mod tests {
    ...
    fn test_truncate_output() {
        ...
        assert!(truncated.contains("[MYCELIUM:PASSTHROUGH]"));
    ...
    fn test_truncate_output_multibyte() {
        ...
        assert!(result.contains("[MYCELIUM:PASSTHROUGH]"));
