//! Runs a shell command and produces a heuristic token-compressed summary of its output.
use crate::tracking;
...
pub fn run(command: &str, verbose: u8) -> Result<()> {
    ...
    println!("{}", summary);
    timer.track(command, "mycelium summary", &raw, &summary);
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn test_detect_output_type() {
        assert!(matches!(
            detect_output_type("10 passed, 2 failed", "cargo test"),
            OutputType::TestResults
        ));
        assert!(matches!(
            detect_output_type("Compiling foo v1.0", "cargo build"),
            OutputType::BuildOutput
        ));
        assert!(matches!(
            detect_output_type("[info] server started\nerror: boom", "myapp"),
            OutputType::LogOutput
        ));
        assert!(matches!(
            detect_output_type("{\"key\": \"value\"}", "curl"),
            OutputType::JsonOutput
        ));
    }
    ...
    fn test_extract_number() {
        assert_eq!(extract_number("10 passed", "passed"), Some(10));
        assert_eq!(extract_number("3 failed tests", "failed"), Some(3));
