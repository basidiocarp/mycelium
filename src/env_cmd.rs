//! Filtered environment variable display that redacts sensitive values.
use crate::tracking;
use anyhow::Result;
...
pub fn run(filter: Option<&str>, show_all: bool, verbose: u8) -> Result<()> {
    ...
    let out = format!("{} vars -> {} shown", total, shown);
    timer.track("env", "mycelium env", &raw, &out);
    Ok(())
}
...
mod tests {
    ...
    fn test_mask_value_long() {
        let masked = mask_value("my-secret-token-12345");
