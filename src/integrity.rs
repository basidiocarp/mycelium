...
//!
//! Mycelium installs a PreToolUse hook (`mycelium-rewrite.sh`) that auto-approves
...
//! - Manual verification via `mycelium verify`
//!
...
pub enum IntegrityStatus {
    ...
    Verified,
    /// Hash mismatch — hook has been modified outside of `mycelium init`
    Tampered { expected: String, actual: String },
    /// Hook exists but no stored hash (installed before integrity checks)
    NoBaseline,
    /// Neither hook nor hash file exist (Mycelium not installed)
    NotInstalled,
...
pub fn run_verify(verbose: u8) -> Result<()> {
    ...
    for hook_path in managed_hook_paths()? {
        ...
        if verbose > 0 {
            eprintln!("[mycelium] Hook:  {}", hook_path.display());
            eprintln!("[mycelium] Hash:  {}", hash_file.display());
        }
        ...
        match verify_hook_at(&hook_path)? {
            ...
            IntegrityStatus::Tampered { expected, actual } => {
                found_hook = true;
                tampered = true;
                eprintln!("[mycelium] FAIL  hook integrity check FAILED");
                eprintln!("[mycelium]");
                ...
                eprintln!("[mycelium]   To inspect: cat {}", hook_path.display());
            }
            ...
            IntegrityStatus::OrphanedHash => {
                found_hook = true;
                eprintln!("[mycelium] WARN  hash file exists but hook is missing");
                eprintln!("[mycelium]       Run `mycelium init -g` to reinstall.");
                eprintln!("[mycelium]       {}", hook_path.display());
            }
        }
    }
    ...
    for hook_path in managed_hook_paths()? {
        match verify_hook_at(&hook_path)? {
            ...
            IntegrityStatus::Tampered { expected, actual } => {
                ...
                eprintln!(
                    "[mycelium]   Expected hash: {}...",
                    expected.get(..16).unwrap_or(&expected)
                );
                eprintln!(
                    "[mycelium]   Actual hash:   {}...",
                ...
                eprintln!("[mycelium]");
                eprintln!("[mycelium]   To restore:  mycelium init -g --auto-patch");
                eprintln!("[mycelium]   To inspect:  mycelium verify");
                std::process::exit(1);
            }
            IntegrityStatus::OrphanedHash => {
                eprintln!("[mycelium] warning: hash file exists but hook is missing");
                eprintln!("[mycelium]   Run `mycelium init -g` to reinstall.");
                eprintln!("[mycelium]   Missing hook: {}", hook_path.display());
...
mod tests {
    ...
    fn test_verify_orphaned_hash() {
        let temp = TempDir::new().unwrap();
        let hook = temp.path().join("mycelium-rewrite.sh");
        let hash_file = temp.path().join(".mycelium-hook.sha256");

        // Create hash but no hook
        fs::write(
            &hash_file,
            "a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2  mycelium-rewrite.sh\n",
        )
        .unwrap();
    ...
    fn test_store_hash_creates_sha256sum_format() {
        ...
        // Format: "<64 hex chars>  mycelium-rewrite.sh\n"
        assert!(content.ends_with("  mycelium-rewrite.sh\n"));
        let parts: Vec<&str> = content.trim().splitn(2, "  ").collect();
        assert_eq!(parts.len(), 2);
        assert_eq!(parts[0].len(), 64);
        assert_eq!(parts[1], "mycelium-rewrite.sh");
    }
    ...
    fn test_invalid_hash_file_rejected() {
        ...
        fs::write(&hook, "test").unwrap();
        fs::write(&hash_file, "not-a-valid-hash  mycelium-rewrite.sh\n").unwrap();
