...
use super::types::{
    classify_error, command_similarity, extract_base_command, is_command_error, is_tdd_cycle_error,
};

// Re-export types so existing callers (report.rs, mod.rs) don't need to change.
pub use super::types::{CorrectionPair, CorrectionRule};
    ...
    fn test_deduplicate_corrections_keeps_distinct() {
        let pairs = vec![
            ...
            CorrectionPair {
                ...
                error_type: ErrorType::Other("Wrong Syntax".to_string()),
                confidence: 0.7,
            },
        ];
