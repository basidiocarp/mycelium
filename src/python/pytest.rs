//! Pytest test runner filter using a state machine text parser to show only failures.
...
/// Run pytest and filter output using a state machine to show only failures.
pub fn run(args: &[String], verbose: u8) -> Result<()> {
    let (tool, mut full_args) = if which_command("pytest").is_some() {
        ("pytest".to_string(), args.to_vec())
    } else {
        let mut a = vec!["-m".to_string(), "pytest".to_string()];
        a.extend_from_slice(args);
        ("python".to_string(), a)
    };

    // Inject short-traceback and quiet flags if not already in user args
    if !args.iter().any(|a| a.starts_with("--tb")) {
        full_args.push("--tb=short".to_string());
    }
