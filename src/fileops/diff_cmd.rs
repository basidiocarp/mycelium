...
pub fn run(file1: &Path, file2: &Path, verbose: u8) -> Result<()> {
    ...
    for change in diff.changes.iter().take(50) {
        match change {
            ...
            DiffChange::Modified(ln, old, new) => out.push_str(&format!(
                "~{:4} {} → {}\n",
                ln,
                truncate(old, 70),
                truncate(new, 70)
    ...
    if diff.changes.len() > 50 {
        out.push_str(&format!("... +{} more changes", diff.changes.len() - 50));
    }
...
pub fn run_stdin(_verbose: u8) -> Result<()> {
    ...
    timer.track("diff (stdin)", "mycelium diff (stdin)", &input, &condensed);
