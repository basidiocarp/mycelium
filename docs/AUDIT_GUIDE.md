# Mycelium Token Savings Audit Guide

Complete guide to analyzing your mycelium token savings with temporal breakdowns and data exports.
...
# Temporal breakdowns
mycelium gain --daily          # All days since tracking started
mycelium gain --weekly         # Aggregated by week
mycelium gain --monthly        # Aggregated by month
mycelium gain --all            # Show all breakdowns at once

# Export formats
mycelium gain --all --format json > savings.json
mycelium gain --all --format csv > savings.csv

# Combined flags
mycelium gain --graph --history --quota    # Classic view with extras
mycelium gain --daily --weekly --monthly   # Multiple breakdowns
```
...
mycelium gain --weekly --format csv > reports/week-$(date +%Y-%W).csv

# Compare this week vs last week
mycelium gain --weekly | tail -3
```
...
mycelium gain --monthly --format json | jq '.monthly[] |
...
# Get CSV data
result = subprocess.run(['mycelium', 'gain', '--all', '--format', 'csv'],
...
1. Export CSV: `mycelium gain --all --format csv > mycelium-data.csv`
2. Open in Excel
...
0 0 * * * mycelium gain --all --format json > /var/www/dashboard/mycelium-stats.json
...
Output Tokens   = estimate_tokens(mycelium_filtered_output)
...
|---------|----------------|-----------|
| `mycelium git status` | 77-93% | Compact stat format |
| `mycelium eslint` | 84% | Group by rule |
| `mycelium vitest run` | 94-99% | Show failures only |
| `mycelium find` | 75% | Tree format |
| `mycelium pnpm list` | 70-90% | Compact dependencies |
| `mycelium grep` | 70% | Truncate + group |
...
```bash
# Location
ls -lh ~/.local/share/mycelium/history.db

# Schema
sqlite3 ~/.local/share/mycelium/history.db ".schema"

# Recent records
sqlite3 ~/.local/share/mycelium/history.db \
  "SELECT timestamp, mycelium_cmd, saved_tokens FROM commands
...
```bash
# Backup
cp ~/.local/share/mycelium/history.db ~/backups/mycelium-history-$(date +%Y%m%d).db

# Restore
cp ~/backups/mycelium-history-20260128.db ~/.local/share/mycelium/history.db

# Export for migration
sqlite3 ~/.local/share/mycelium/history.db .dump > mycelium-backup.sql
```
...
# Reset all data
rm ~/.local/share/mycelium/history.db
# Next mycelium command will recreate database
```
...
```yaml
# .github/workflows/mycelium-stats.yml
name: Mycelium Stats Report
on:
  schedule:
  ...
  stats:
    ...
    steps:
      ...
      - name: Generate report
        run: |
          mycelium gain --weekly --format json > stats/week-$(date +%Y-%W).json
      - name: Commit stats
        run: |
          git add stats/
          git commit -m "Weekly mycelium stats"
          git push
```
...
def send_mycelium_stats():
    result = subprocess.run(['mycelium', 'gain', '--format', 'json'],
                           capture_output=True, text=True)
    data = json.loads(result.stdout)

    message = f"""
    📊 *Mycelium Token Savings Report*
...
# Check record count
sqlite3 ~/.local/share/mycelium/history.db "SELECT COUNT(*) FROM commands"
...
mycelium gain --format json 2>&1 | tee /tmp/mycelium-debug.log | jq .

# Use release build to avoid warnings
cargo build --release
./target/release/mycelium gain --format json
```
