...
impl<I: Iterator<Item = Token>, B: Builder> Parser<I, B> {
    ...
    pub fn complete_command(&mut self) -> ParseResult<Option<B::Command>, B::Error> {
        ...
        if self.iter.peek().is_some() {
            Ok(Some(try!(
                self.complete_command_with_leading_comments(pre_cmd_comments)
            )))
        } else {
    ...
    fn complete_command_with_leading_comments(
        ...
        Ok(try!(self.builder.complete_command(
            pre_cmd_comments,
            cmd,
            sep,
            cmd_comment
    ...
    pub fn redirect(&mut self) -> ParseResult<Option<Result<B::Redirect, B::Word>>, B::Error> {
        ...
        let redir_tok = match self.iter.peek() {
            ...
            Some(&DLess) | Some(&DLessDash) => {
                return Ok(Some(Ok(try!(self.redirect_heredoc(src_fd)))))
            }
        ...
        macro_rules! get_dup_path {
            ($parser:expr) => {{
                let path = if $parser.peek_reserved_token(&[Dash]).is_some() {
                    ...
                    let path =
                        if let Some(p) = try!($parser.word_preserve_trailing_whitespace_raw()) {
                            p
                        } else {
        ...
        match self.iter.next() {
            ...
            Some(Dollar) => match self.iter.peek() {
                ...
                | Some(&Dash) | Some(&At) | Some(&Name(_)) => {
                    Ok(SimpleWordKind::Param(try!(self.parameter_inner())))
                }
    ...
    pub fn do_group(&mut self) -> ParseResult<builder::CommandGroup<B::Command>, B::Error> {
        ...
        try!(self
            .reserved_word(&[DONE])
            .or_else(|()| Err(ParseError::IncompleteCmd(
                DO,
                start_pos,
                DONE,
                self.iter.pos()
    ...
    pub fn function_declaration(&mut self) -> ParseResult<B::PipeableCommand, B::Error> {
        ...
        Ok(try!(self.builder.function_declaration(
            name,
            post_name_comments,
            body
        )))
    ...
    fn command_group_internal(
        ...
        loop {
            ...
            cmds.push(try!(
                self.complete_command_with_leading_comments(leading_comments)
            ));
        }
